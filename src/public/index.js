...
window.onload = () => {
  ...
  flatpickr("#fechaHoraInicio", {
    ...
    onChange: (selectedDates) => {
      fechaHoraInicioValue = selectedDates[0]?.toISOString() ?? null;
    },
  });
  ...
  flatpickr("#fechaHoraFin", {
    ...
    position: "right",
    onChange: (selectedDates) => {
      fechaHoraFinValue = selectedDates[0]?.toISOString() ?? null;
    },
  });
};

const form = document.querySelector("form");
const btnGenerarPDF = document.getElementById("pdfView");
const comentario = document.getElementById("comentario");
const nombre = document.getElementById("nombre");

btnGenerarPDF.onclick = async (e) => {
  e.preventDefault();
  ...
  try {
    ...
    if (!resDatos.ok) throw new Error(`Error parámetros: ${resDatos.status}`);
    const result = await resDatos.json();
    ...
    alert(
      "Ocurrió un error al generar el reporte. Revisa la consola para más detalles."
    );
  } finally {
